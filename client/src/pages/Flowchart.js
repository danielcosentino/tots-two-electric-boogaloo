...
let classes = [
  {
    id: "0",
    classId: "COP2500",
    numUncompletedPrereqs: 0,
    preReqs: [],
    postReqs: [1],
    type: white,
    className: "Concepts in CS",
  },
  {
    id: "1",
    classId: "COP3223",
    numUncompletedPrereqs: 1,
    preReqs: [0],
    postReqs: [2, 3, 4, 5],
    type: grey,
    className: "Intro to C Programming",
  },
  {
    id: "2",
    classId: "COP3330",
    ...
    postReqs: [6],
    type: grey,
    className: "Object-Oriented Prog.",
  },
  {
    id: "3",
    classId: "COP3502",
  ...
  },
  {
    ...
    postReqs: [],
    type: grey,
    className: "Security in Computing",
  },
  {
    id: "6",
    classId: "COP3503",
    numUncompletedPrereqs: 3,
    preReqs: [2, 3, 9],
    postReqs: [14, 15],
    type: grey,
  ...
  },
  {
    ...
    postReqs: [14, 15, 16],
    type: grey,
    className: "Foundation Exam",
  },
  {
    id: "8",
    classId: "COP3402",
    numUncompletedPrereqs: 2,
    preReqs: [3, 4],
    postReqs: [16],
    type: grey,
    className: "Systems Software",
  },
  {
    id: "9",
    classId: "COT3100",
  ...
  },
  {
    ...
    postReqs: [],
    type: grey,
    className: "Physics II with Calc",
  },
  {
    id: "14",
    classId: "COT4210",
  ...
  {
    id: "22",
    classId: "Extra Math 1",
    numUncompletedPrereqs: 0,
    preReqs: [],
    postReqs: [],
    type: white,
    className: "Extra Math I",
  },
  {
    id: "23",
    classId: "Extra Math 2",
    numUncompletedPrereqs: 0,
    preReqs: [],
    postReqs: [],
    type: white,
    className: "Extra Math II",
  },
  {
    id: "24",
    classId: "GEP 3",
...
let initialNodes = [
  ...
  {
    id: classes[31].id,
    data: { label: classes[31].className },
    style: classes[31].type,
    type: "input",
    position: { x: 70, y: 410 },
  },
  {
...
const Flowchart = () => {
  ...
  useEffect(() => {
    console.log("all the completed classes", completed);
  }, [completed]);
  ...
  function changeColor(id) {
    ...
    // Active -> clickActive
      ...
      classes[id].postReqs.forEach((currentPostreq) => {
        ...
        classes[currentPostreq].type =
          classes[currentPostreq].numUncompletedPrereqs <= 0 ? white : grey;
      });
    ...
    // clickActive -> active (green -> white)
      ...
      classes[id].preReqs.forEach((currentPrereq) => {
        classes[currentPrereq].type = classes[currentPrereq].postReqs.some(
          (currentPostreq) =>
            classes[currentPostreq].type === lightGreen ||
            classes[currentPostreq].type === darkGreen
        )
          ? darkGreen
          : lightGreen;
      });
      ...
      setCompleted((prevCompleted) =>
        prevCompleted.filter((x) => x !== classes[id].classId)
      );
    ...
    setNodes((nds) =>
      nds.map((node) => {
        ...
        if (node.type !== "group") node.style = classes[node.id].type;
        // if (node.id === id) {
  ...
  const [mathSciCount, setMathSciCount] = useState(0);
  ...
  const getElectives = async () => {
    try {
      fetch(process.env.REACT_APP_API_URL + "/api/getElectives", {
        method: "GET",
  ...
  const handleMathSciCountChange = (event) => {
    setMathSciCount(event.target.value);
  };
  ...
  const handleScheduleSubmit = async () => {
    ...
    for (let i = 1; i < 13; i++)
    {
      if (completed.indexOf("GEP " + i) === -1)
      {
        gepsCompleted.push(false);
      }
      else
      {
    ...
    let obj = {
      nextSemSeason: season,
      nextSemYear: year,
      completedClasses: completed,
      selectedElectives: chosenElectives,
      geps: gepsCompleted,
      mathSciCount: mathSciCount
    };
    ...
    {
      fetch(
        process.env.REACT_APP_API_URL + "/api/generateSchedule",
      ...
      ).then(async res=>
      {
        ...
        let body = await res.json();
        console.log("this is the body for generateSchedule", body);
        if (body.error)
        {
          if (body.error === "it broke"
           || body.error === "Schedule could not be added to user")
          {
            console.log("An error occured while generating your schedule");
        ...
        else // schedule generated correctly
        {
          if (body.schedule.length === 0)
          {
            console.log("The schedule was not returned in the response correctly");
          ...
          <div className="no-borderRadius">
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                value={mathSciCount}
                label="MathSciCount"
                onChange={handleMathSciCountChange}
                sx={{
                  maxWidth: MAXWIDTH,
                  maxHeight: MAXHEIGHT,
                }}
            ...
            <Box sx={{ display: "flex", justifyContent: "center" }}>
              ...
              <FormControl fullWidth>
                ...
                >
                  {years
                    ? years.map((yearChoice) => {
                        return (
                          <MenuItem value={yearChoice}>{yearChoice}</MenuItem>
                        );
                      })
                    : null}
                </Select>
